...
- Law
  ...
  - 隱私權(包含以下法規):
    ...
    - 個資法:
      ...
      - **當事人之權利**:請求停止蒐集個資利用個資,請求給予複製本,請求修改或補充,請求查詢個資,
      - ***個資公開需要遵守法規即使當事人同意也不行!***
...
- 風險管理
  ```
  目的:根據資料的機密性,完整性,可用性來列出組織可能遇到的資產風險然後根據風險的類別做出相對應的改善措施。
  例如說組織的web應用算是資訊資產,組織為了防止資產風險所以會根據不一樣的弱點(OWASP TOP TEN)來做出相對應的風險修改手段。原始碼檢測,設置防火牆,制定新的存取政策,甚至是連買保險都是一種風險修改的手段!
  ```
  - ISO27005風險管理流程
    - **STEP1 | 全景建立 |** :包括目標,範圍,參與人員等等。
    - **STEP2 | 風險鑑別 |** :根據CIA及法規遵循來風險評鑑。包括風險識別,風險分析(決定風險等級),風險評估。如果不符合預期目標可以一直重複風險評鑑。
    - **STEP3 | 內部溝通或外部諮商 |** :如果關乎到改變組織政策,或有多個利益方或組織重大影響就必須先內部內部溝通再外部風險諮詢,可以找顧問或顧問公司。
    - **STEP4 | 產生殘餘風險 |** :經過內部討論或外部諮詢後會產生一個殘餘風險。
    - **STEP5 | 分析殘餘風險 |** :經過內部討論或外部諮詢後的組織殘餘風險需要再分析是否合理。
    - **STEP4 | 最後風險處理 |** :風險轉移(買保險,丟給第三方),風險接受(為組織可接受之風險級別),風險降低,風險避免。例如買保險,增設防火牆,修改組織政策,安全測試等等。
...
- 身分存取管理
    ...
    - **零信任架構 (Zero Trust Architecture, ZTA)**
      - 是一個用來確保資料的機敏性及完整性及管理網路權限存取的框架
      - 核心:在使用者取得授權之前拒絕任何連線,無論是內部網路還是外部網路。
      - NIST SP 800-207
      - 主要核心:先知道使用者的身分(身分鑑別),再來判斷使用者是否可信任(信任判斷),檢查使用者的設備之安全狀態(設備鑑別)
      - 適用場景:遠端連線,雲端連線,vpn連線
...
- 資安應變及通報辦法
  - 標準資安通報流程
    ...
    - **STEP3 | 紀錄細節 |** :紀錄入侵者的入侵蹤跡
    - **STEP4 | 啟動應變 |** :開始阻擋入侵者
    - **STEP5 | 深入調查 |** :釐清為甚麼被入侵
    - **STEP6 | 災難復原計畫(BCP) |**  :開始復原系統至可運行狀態及透過備份檔來恢復機敏數據
    - **STEP7 | 結案報告 |** :紀錄整體事件並根據原因提出修補或改善方案。
